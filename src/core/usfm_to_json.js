...
const toJson = (options, callback) => {
  ...
  try {
    var lineReader = window.readline.createInterface({
      input: fs.createReadStream(options.usfmFile),
    });
    patterns = fs.readFileSync(path.join(appPath, `patterns.prop`), "utf8");
    var book = {},
      verse = [],
      db = require(`${__dirname}/../core/data-provider`).targetDb(),
      refDb = require(`${__dirname}/../core/data-provider`).referenceDb(),
      c = 0,
      v = 0,
      vnum = 0,
      usfmBibleBook = false,
  ...
  lineReader.on("line", function (line) {
    ...
    // }
      ...
      let cleanedStr = replaceMarkers(line);
      cleanedStr = "\n" + cleanedStr;
      book.chapters[c - 1].verses[vnum - 1].verse +=
