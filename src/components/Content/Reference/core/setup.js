...
const SetUp = () => {
        ...
        refDb.get("ref_history").then(function(doc) {
            ...
            db.get(AutographaStore.bookId).then(function(doc) {
                refDb.get('refChunks').then(function(chunkDoc) {
                    ...
                    getRefContents(AutographaStore.activeRefs[0]+'_'+Constant.bookCodeList[parseInt(AutographaStore.bookId, 10) - 1],chapter.toString());
                })
            })
        }).catch(function(err) {
    ...
    const getRefContents = (id,chapter) => {
        refDb.get('targetReferenceLayout').then((doc) => {
            ...
            getContent(AutographaStore.activeRefs[0]+'_'+Constant.bookCodeList[parseInt(AutographaStore.bookId, 10) - 1],chapter).then((content)=>{
    ...
    const handleRefChange = (refDropDownPos, event) => {
        ...
        session.defaultSession.cookies.get({ url: 'http://book.autographa.com' }, (error, bookCookie) => {
            if(bookCookie.length > 0){
                getRefContents(referenceValue+'_'+Constant.bookCodeList[parseInt(bookCookie[0].value, 10) - 1],AutographaStore.chapterId) 
            }else{
                getRefContents(referenceValue+'_'+Constant.bookCodeList[parseInt('1', 10) - 1],AutographaStore.chapterId)
            }    
        })
