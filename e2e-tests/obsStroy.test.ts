...
// test('Edit the heading tilte ', async () => {
// 	await window.getByText('1. The Creation').fill("1. The Creation edit");
// 	const editorpane = await window.innerText('[aria-label=editor-pane]');
// 	expect(editorpane).toBe('EDITOR');
// });

// test('add text in the verse 1 and  ', async () => {
...
// test('Editing the existing project page', async () => {
...
// 	await window.getByText('test version').fill('edittesting')
// 	await window.locator('input[name="version_abbreviated"]').fill('oe')
...
test('Update the Urdu project', async () => {
	...
	console.log(table, await headers.allTextContents())
	...
	const nameMatch = rows.filter({
		has: window.locator('td'),
		hasText:"urdu project"
	})

	console.log(await nameMatch)
