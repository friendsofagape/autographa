  ...
  const translationBurritoChecksAndCreation = async () => {
    ...
    if (checkCanon === false) {
      ...
      await createVersificationUSFM(
        ...
        scope,
        projectMetaObj.language?.scriptDirection || projectMetaObj.language?.ld,
        projectMetaObj.language?.code || projectMetaObj.language?.lc,
        id,
        ...
        const burritoFile = await createTranslationSB(
          ...
          projectMetaObj.newProjectFields,
          scope,
          projectMetaObj.language?.title || projectMetaObj.language?.ang,
          projectMetaObj.copyright,
          id,
          projectMetaObj.project,
          projectMetaObj.call,
